...
function Breed() {
  ...
  const breed = async () => {
    try {
      ...
      toast.error("Breeding Cancelled");
  ...
  const showBreedNft = async () => {
    try {
      ...
      let walletUris = await breedContract.methods
        .tokenURI(walletIds[0])
        .call();
  ...
  const getPrice = async () => {
    let price = await axios.get(
      "https://min-api.cryptocompare.com/data/price?fsym=USDT&tsyms=BNB"
    );
                          ...
                          <div className="col-12 d-flex flex-row mt-3">
                            <div className="ms-3">
                              {!playing ? (
                                ...
                                <img src={On} onClick={playingSound} />
                              )}
                            </div>
